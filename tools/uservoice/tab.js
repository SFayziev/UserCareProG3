...
UserVoice.Dialog = {
    ...
    }" : "") + ((UserVoice.Util.isIE() && (UserVoice.Util.isIE(function(v) {
        return v < 7
    }) || (UserVoice.Util.isIE(function(v) {
        return v >= 7
    }) && UserVoice.Util.isQuirksMode()))) ? "\
    #uservoice-overlay,\
    #uservoice-dialog {\
      position: absolute;\
    }\
    ...
    #uservoice-dialog #uservoice-dialog-close {\
      background: none;\
      filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src='https://uservoice.com/pkg/icons/close.png');\
    }\
    .dialog-open select {\
      visibility: hidden;\
    }\
    ...
    element: function() {
        if (!document.getElementById(this.id)) {
            ...
            dummy.innerHTML = '<div id="' + this.id + '" class="uservoice-component" style="display:none;">' + '<a href="#close" onclick="UserVoice.Dialog.close(); return false;" id="' + this.id + '-close" title="Close Dialog"><span style="display: none;">Close Dialog</span></a>' + '<div id="' + this.id + '-content"></div></div>';
...
UserVoice.Util.includeCss(UserVoice.Util.render(UserVoice.Dialog.css_template, {
    background_image_url: UserVoice.Util.getAssetHost() + '/images/widgets/close.png'
}));
if (!UserVoice) {
    var UserVoice = {};
}
...
(function() {
    ...
    if (typeof UserVoice.JSON.stringify !== 'function') {
        UserVoice.JSON.stringify = function(value, replacer, space) {
            var i;
            gap = '';
            indent = '';
    ...
    if (typeof UserVoice.JSON.parse !== 'function') {
        UserVoice.JSON.parse = function(text, reviver) {
            var j;
...
UserVoice.Logger = {
    ...
    warning: function(message) {
        this._log("UserVoice WARNING: " + message);
    },
    error: function(message) {
        this._log("UserVoice ERROR: " + message);
        alert("UserVoice ERROR: " + message);
    }
};
UserVoice.Util = {
    sslAssetHost: "https://assets.uvcdn.com",
...
UserVoice.Popin = {
    content_template: '<iframe id="uservoice_dialog_iframe" src="#{url}/widgets/#{dialog}.html?#{query}" frameborder="0" scrolling="no" allowtransparency="true" width="#{width}" height="#{height}" style="height: #{height}; width: #{width};"></iframe>',
    ...
    setupOptions: function(options) {
        ...
        if (options.key == null && options.host == null) {
            UserVoice.Logger.error("'host' must be set.");
            UserVoice.Logger.error("'key' must be set.");
        } else if (options.key == null) {
            UserVoice.Logger.warning("'key' must be set for the widget to work with SSL.");
        }
        if (options.forum == null) {
            UserVoice.Logger.error("'forum' must be set.");
        }
        if (!options.params) {
            options.params = {};
        }
    ...
    preload: function(options) {
        this.setupOptions(options);
        UserVoice.Dialog.preload(UserVoice.Util.render(this.content_template, this.getContext()));
    },
    show: function(options) {
        this.setupOptions(options);
        UserVoice.Dialog.show(UserVoice.Util.render(this.content_template, this.getContext()));
        try {
            var iframeElement = document.getElementById("uservoice_dialog_iframe").contentWindow;
            iframeElement.location = UserVoice.Util.render(this.opened_url_template, this.getContext());
        } catch (e) {
            UserVoice.Logger.warning("Error sending the 'open' notification");
            UserVoice.Logger.warning(e);
        }
    },
    getContext: function() {
        var context = {
        ...
        if (context.params.custom_fields && (typeof context.params.custom_fields == "object")) {
            context.params.custom_fields = encodeURIComponent(UserVoice.JSON.stringify(context.params.custom_fields));
        }
        context.query = UserVoice.Util.toQueryString(context.params);
        return context;
    },
    getReferer: function() {
    ...
    url: function() {
        ...
        if ("https:" == document.location.protocol && this.options.key != null) {
            url = 'https://' + this.options.key + '.uservoice.com/forums/' + this.options.forum;
        } else {
            url = 'http://' + this.options.host + '/forums/' + this.options.forum;
        }
        return url;
    }
};
UserVoice.Tab = {
    ...
    show: function(options) {
        ...
        if (!this.options.no_dialog) {
            html += '" onclick="UserVoice.Popin.show(); return false;"';
            if (this.options.preload) {
                html += '" onmouseover="UserVoice.Popin.preload();"';
            }
        }
        html += ' href="' + UserVoice.Popin.url() + '?' + UserVoice.Popin.getContext().query + '">' + (this.options.tab_string[this.options.lang] ? this.options.tab_string[this.options.lang] : 'Open Feedback Dialog') + '</a>';
    ...
    setupOptions: function(options) {
        ...
        this.options.text_url = UserVoice.Util.getAssetHost() + '/images/widgets/' + (this.options.tab_string[this.options.lang] ? this.options.lang : 'en') + '/feedback_tab_' + this.options.text_color + '.png';
