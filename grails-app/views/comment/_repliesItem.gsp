            ...
            <div class=" media-body">
                ...
                <div class="topic-votes pull-right">
                    <g:if test="${comment?.statusDTO}">
                        <span class="margin-left-5 rounded  label" style="background: ${comment?.statusDTO?.color}" ><locale:message  proj="${project}" code="article.status.name.${comment?.statusDTO?.id}" default="${status?.name}" /> </span>
                    </g:if>
                </div>
                <div class="topic-votes pull-right">
