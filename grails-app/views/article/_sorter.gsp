...
<div class="btn-group">
    ...
    <ul class="dropdown-menu" role="menu">
        <li><a data-action="listByStatus" onclick="setArticleListStatus(${module?.id}, -1)"> All <span class="badge  badge-blue rounded-x">${pageCount?.getCountByStatus(-1 )}</span></a></li>
        <li class="divider"></li>
        <li><a   data-action="listByStatus" data-status-value="0" onclick="setArticleListStatus(${module?.id}, 0 )">  Unmarked <span class="badge badge-blue rounded-x">${pageCount?.getUnmarkedTopic()}</span></a></li>
        <li><a   data-action="listByStatus" data-status-value="-10" onclick="setArticleListStatus(${module?.id}, -10 )">  Active <span class="badge badge-blue rounded-x">${pageCount?.getActiveTopic()}</span></a></li>

        <g:each in="${topicStatuses}" var="forumStatus" >
           <g:if test="${forumStatus.articleStatusDTO?.logicalgroup==0}"> <li><a   style="padding-left:20px;white-space:nowrap;" data-action="listByStatus" data-status-value="${forumStatus.articleStatusDTO?.id}" onclick="setArticleListStatus(${module?.id}, ${forumStatus.articleStatusDTO?.id})"> <locale:message  proj="${UCproject}" code="article.status.name.${forumStatus.articleStatusDTO?.id}" default="${forumStatus.articleStatusDTO?.name}" />  <span class="badge badge-blue rounded-x">${pageCount?.getCountByStatus(forumStatus.articleStatusDTO?.id )}</span></a></li></g:if>
        </g:each>

        <li class="divider"></li>
        <li><a   data-action="listByStatus" data-status-value="-20" onclick="setArticleListStatus(${module?.id}, -20 )">  Closed <span class="badge badge-blue rounded-x">${pageCount?.getClosedTopic() }</span></a></li>
        <g:each in="${topicStatuses}" var="forumStatus" >
            <g:if test="${forumStatus.articleStatusDTO?.logicalgroup==1}"> <li><a   style="padding-left:20px;white-space:nowrap;" data-action="listByStatus" data-status-value="${forumStatus.articleStatusDTO?.id}" onclick="setArticleListStatus(${module?.id}, ${forumStatus.articleStatusDTO?.id})"> <locale:message  proj="${UCproject}" code="article.status.name.${forumStatus.articleStatusDTO?.id}" default="${forumStatus.articleStatusDTO?.name}" />  <span class="badge badge-blue rounded-x">${pageCount?.getCountByStatus(forumStatus.articleStatusDTO?.id )}</span></a></li></g:if>
        </g:each>
