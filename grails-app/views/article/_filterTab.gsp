...
<g:each in="${forumTypes}" var="topicTypeDTO">
    <li class="<g:if test="${catalogParams?.type==topicTypeDTO?.id}">active</g:if> "> <a  href="#" data-action="listByType" data-type-value="${topicTypeDTO?.id}" onclick="setArticleListType(${module?.id}, ${topicTypeDTO?.id}); return false;" >
